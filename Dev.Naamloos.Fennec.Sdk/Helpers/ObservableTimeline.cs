...
public sealed class ObservableTimeline : ObservableRangeCollection<TimelineItem>, IDisposable
{
    ...
    public bool IsLoadingHistory
    {
        ...
        private set
        {
            ...
            OnPropertyChanged(
                new System.ComponentModel.PropertyChangedEventArgs(nameof(IsLoadingHistory))
            );
        }
    }
    ...
    public bool HasReachedStart
    {
        ...
        private set
        {
            ...
            OnPropertyChanged(
                new System.ComponentModel.PropertyChangedEventArgs(nameof(HasReachedStart))
            );
        }
    }
    ...
    private async Task RestartListenerAsync()
    {
        ...
        {
            Debug.WriteLine($"Failed to restart timeline listener: {exception}");
        }
        finally
        {
    ...
    private void ApplyUpdate(TimelineDiff update)
    {
        switch (update)
        {
            ...
            default:
                Debug.WriteLine($"Unhandled timeline diff: " + update.GetType().FullName);
                break;
        }
    }
    ...
    private void ApplyInsert(TimelineDiff.Insert insert)
    {
        ...
        {
            Debug.WriteLine(
                $"Ignoring timeline insert at invalid index {index}. " + $"Current count: {Count}."
            );
    ...
    private void ApplySet(TimelineDiff.Set set)
    {
        ...
        {
            Debug.WriteLine(
                $"Ignoring timeline set at invalid index {index}. " + $"Current count: {Count}."
            );
    ...
    private void ApplyRemove(TimelineDiff.Remove remove)
    {
        ...
        {
            Debug.WriteLine(
                $"Ignoring timeline remove at invalid index {index}. " + $"Current count: {Count}."
            );
    ...
    public void Dispose()
    {
        ...
        {
            Debug.WriteLine($"Failed to cancel timeline listener: {exception}");
        }
        ...
        {
            Debug.WriteLine($"Failed to cancel pagination status listener: {exception}");
        }
