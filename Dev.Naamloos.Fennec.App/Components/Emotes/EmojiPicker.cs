...
public sealed partial class EmojiPicker : ContentView
{
    ...
    public EmojiPicker()
    {
        ...
        var categories = new CollectionView
        {
            ...
            ItemTemplate = new DataTemplate(() =>
                new Label
                {
                    ...
                    .Bind(Label.TextProperty, nameof(EmojiCategory.Icon))
                    .Bind(Label.FontFamilyProperty, nameof(EmojiFontFamily), source: this)
                    .Bind(SemanticProperties.DescriptionProperty, nameof(EmojiCategory.Name))
            ),
        };
        categories.SelectionChanged += (_, args) =>
            SelectedCategory = args.CurrentSelection.FirstOrDefault() as EmojiCategory;
        categories.SetBinding(
            ItemsView.ItemsSourceProperty,
            new Binding(nameof(Categories), source: this)
        );
        categories.SetBinding(
            SelectableItemsView.SelectedItemProperty,
            new Binding(nameof(SelectedCategory), source: this)
        );
        ...
        _grid.SetBinding(
            ItemsView.ItemsSourceProperty,
            new Binding(nameof(VisibleItems), source: this)
        );
        ...
        Content = new Grid
        {
            ...
            Children =
            {
                new SearchBar { Placeholder = "Search emoji" }
                    ...
                    .Bind(SearchBar.FontFamilyProperty, nameof(EmojiFontFamily), source: this)
                    .Row(0),
                categories.Row(1),
                _grid.Row(2),
                new ActivityIndicator { HeightRequest = 28, IsRunning = true }
                    .Bind(IsVisibleProperty, nameof(IsLoadingCustomEmotes), source: this)
                    .Row(3),
            },
        };
    ...
    private static void OnModeChanged(BindableObject bindable, object oldValue, object newValue) =>
        _ = ((EmojiPicker)bindable).ConfigureCategoriesAsync();
    ...
    private static void OnSelectedCategoryChanged(
        BindableObject bindable,
        object oldValue,
        object newValue
    ) => ((EmojiPicker)bindable).ScheduleFilter();
    ...
    private async Task LoadUnicodeAsync()
    {
        ...
        catch (Exception exception)
        {
            System.Diagnostics.Debug.WriteLine(
                $"Could not load bundled Unicode emoji: {exception}"
            );
        }
    }
    ...
    private static EmojiItem CreateCustomItem(MatrixEmote emote)
    {
        ...
        {
            Id = $"matrix:{pack}:{emote.Name}",
            Kind = EmojiKind.MatrixCustom,
            Name = name,
            Shortcode = emote.Name,
            CategoryId = "custom",
            MxcUri = emote.Source,
            PackId = pack,
    ...
    private IReadOnlyList<EmojiItem> Filter(string query, string category)
    {
        var all =
            Mode == EmojiPickerMode.Composer
                ? _unicodeItems.Concat(_customItems).ToArray()
                : _unicodeItems;
        ...
        if (string.IsNullOrEmpty(query))
        {
            if (category == "recent")
                return _usage
                    .RecentEmoji.Select(emoji => all.FirstOrDefault(item => item.Unicode == emoji))
                    .OfType<EmojiItem>()
                    .ToArray();
    ...
    private int Score(EmojiItem item, string query)
    {
        ...
        var score =
            shortcode == query ? 600
            : shortcode.StartsWith(query) ? 500
            : name.StartsWith(query) ? 400
            : item.SearchKey.StartsWith(query) ? 300
            : item.SearchKey.Contains(query) ? 100
            : 0;
    ...
    private void OnEmojiSelected(object? sender, SelectionChangedEventArgs args)
    {
        ...
        if (
            args.CurrentSelection.FirstOrDefault() is not EmojiItem item
            || (
                Mode == EmojiPickerMode.Reaction
                && (item.Kind != EmojiKind.Unicode || !UnicodeEmoji.IsValid(item.Unicode))
            )
        )
            return;
    ...
    {
        private readonly DataTemplate _unicode = new(() =>
            new Label
            {
                FontSize = 25,
                WidthRequest = 46,
                ...
                .Bind(Label.TextProperty, nameof(EmojiItem.Unicode))
                .Bind(Label.FontFamilyProperty, nameof(EmojiFontFamily), source: picker)
                .Bind(SemanticProperties.DescriptionProperty, nameof(EmojiItem.Name))
        );
        private readonly DataTemplate _custom = new(() =>
            new MatrixImage
            {
                IsJson = false,
                UseRoomImageCache = true,
                ThumbnailWidth = 64,
                ThumbnailHeight = 64,
                WidthRequest = 46,
                HeightRequest = 46,
                ...
                .Bind(MatrixImage.MatrixSourceProperty, nameof(EmojiItem.MxcUri))
                .Bind(
                    SemanticProperties.DescriptionProperty,
                    nameof(EmojiItem.Name),
                    stringFormat: "{0}, custom emote"
                )
        );

        protected override DataTemplate OnSelectTemplate(object item, BindableObject container) =>
            ((EmojiItem)item).Kind == EmojiKind.Unicode ? _unicode : _custom;
    }
}
...
public static class EmojiCatalog
{
            ...
            parsed.Add(
                new EmojiItem
                {
                    Id =
                        ...
                        + string.Join(
                            '-',
                            sequence.EnumerateRunes().Select(rune => rune.Value.ToString("x"))
                        ),
        ...
        return parsed
            ...
            .Select(group =>
            {
                ...
                return baseItem is null
                    ...
                    : baseItem with
                    {
                        Variants = group.Where(item => item != baseItem).ToArray(),
                    };
            })
            .OfType<EmojiItem>()
            .ToArray();
    ...
    private static string WithoutSkinTone(string value) =>
        string.Concat(
            value
                .EnumerateRunes()
