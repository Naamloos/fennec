...
public sealed partial class EmoteSettingsView : ContentView
{
    ...
    public EmoteSettingsView()
    {
        ...
        Content = new SettingsSection(
            "Personal stickers and emoji",
            new Label
            {
                Text = "These are available in every chat.",
                Opacity = .7,
                FontSize = 12,
            },
    ...
    private async Task AddEmoteAsync()
    {
        ...
        var name = await InAppDialogs.PromptAsync(
            Shell.Current,
            "Emoji name",
            "Use the name people type between colons.",
            "Continue"
        );
        if (string.IsNullOrWhiteSpace(name) || Emotes.Any(emote => emote.Name == name.Trim()))
            return;
    ...
    {
        if (emote is null || MatrixClient is null)
            return;
