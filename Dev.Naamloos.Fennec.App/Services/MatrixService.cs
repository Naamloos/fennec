...
namespace Dev.Naamloos.Fennec.App.Services
{
    ...
    {
        private const string SessionStorageKey = "matrix.session";

        private const string StoreKeyStorageKey = "matrix.store-key";
        ...
        public bool IsLoggedIn => Client?.UserId() is not null;
        ...
        public async Task<bool> TryRestoreSessionAsync()
        {
            ...
            try
            {
                ...
                var serializedSession = await SecureStorage.Default.GetAsync(SessionStorageKey);
                ...
                Client = await BuildClientAsync(session.UserId);
        ...
        public async Task LoginAsync(string homeserver, string username, string password)
        {
            ...
            try
            {
                ...
                Client = await BuildClientAsync(username, homeserver);
        ...
        private async Task<Client> BuildClientAsync(string userId, string? homeserver = null)
        {
            var accountDirectory = GetAccountDirectory(userId);

            var dataPath = Path.Combine(accountDirectory, "data");

            var cachePath = Path.Combine(accountDirectory, "cache");

            Directory.CreateDirectory(dataPath);
            Directory.CreateDirectory(cachePath);

            var storeKey = await GetOrCreateStoreKeyAsync();

            var storeBuilder = new SqliteStoreBuilder(dataPath, cachePath).Key(storeKey);
            ...
            {
                clientBuilder = clientBuilder.HomeserverUrl(homeserver);
            }
        ...
        private async Task StartSyncAsync()
        {
            if (Client is null)
            {
                throw new InvalidOperationException("The Matrix client is unavailable.");
            }
        ...
        private static string GetAccountDirectory(string userId)
        {
            var safeUserId = Convert
                .ToHexString(SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(userId)))
                .ToLowerInvariant();

            return Path.Combine(FileSystem.AppDataDirectory, "fennec-store", safeUserId);
        }

        private static async Task<byte[]> GetOrCreateStoreKeyAsync()
        {
            var encodedKey = await SecureStorage.Default.GetAsync(StoreKeyStorageKey);
            ...
            if (encodedKey is not null)
            {
                ...
                {
                    throw new InvalidDataException("The Matrix store key is invalid.");
                }
            ...
            await SecureStorage.Default.SetAsync(StoreKeyStorageKey, Convert.ToBase64String(key));
        ...
        public Task<byte[]> GetThumbnailAsync(string url, ulong width, ulong height) =>
            _thumbnailCache
                .GetOrAdd(
                    $"{width}x{height}:{url}",
                    _ => new(() => DownloadThumbnailAsync(url, width, height))
                )
                .Value;
        ...
        public Task<byte[]> GetMediaThumbnailAsync(string sourceJson, ulong width, ulong height) =>
            _thumbnailCache
                .GetOrAdd(
                    $"media:{width}x{height}:{sourceJson}",
                    _ => new(() => DownloadMediaThumbnailAsync(sourceJson, width, height))
                )
                .Value;
        ...
        {
            var client =
                Client ?? throw new InvalidOperationException("Matrix client is not initialized.");
        ...
        public async Task<IReadOnlyList<MatrixDevice>> GetDevicesAsync()
        {
            ...
            var result =
                JsonSerializer.Deserialize<DevicesResponse>(
                    await response.Content.ReadAsStringAsync(),
                    new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
                ) ?? new([]);
            return result
                .Devices.Select(device => new MatrixDevice(
                    device.DeviceId,
        ...
        public async Task<IReadOnlyList<MatrixImageAsset>> GetRoomImagePackAsync(string roomId)
        {
            try
            {
                ...
                foreach (var image in images.EnumerateObject())
                {
                    ...
                    if (!content.TryGetProperty("url", out var urlProperty))
                        continue;
        ...
        private async Task<byte[]> DownloadMediaThumbnailAsync(
            string sourceJson,
            ulong width,
            ulong height
        )
